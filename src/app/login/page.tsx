...
export default function LoginForm() {
  ...
  const handleOAuthSignIn = async (provider: "github" | "google") => {
   {provider === "google" ? setIsLoadingGoogle(true) : setIsLoading(true)}
    try {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-200 dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-sans">
      ...
      <main className="flex-1 flex items-center justify-center p-4">
        <div className="flex  flex-col w-full max-w-5xl items-center justify-between gap-8">
          {/* Left: Branding Section */}
          <motion.div
            initial={{ opacity: 0, x: -50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8, ease: "easeOut" }}
            className="flex-1 mb flex flex-col items-start"
          >
          ...
          <motion.div
            ...
            className="w-full mb-8 max-w-sm p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 transition-all duration-300 hover:shadow-lg"
          >
